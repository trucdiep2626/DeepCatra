...
class Hw(nn.Module):
    ...
    def forward(self, X, neis_embeds, H, dg_list):
        ...
        out = (
            torch.squeeze(torch.matmul(A, torch.unsqueeze(H, 2)), -1) + b
        )  # (N, s, s) * (N, s) + (N, s)
        return out  # (N, s)
...
class AggrSum(nn.Module):
    ...
    def forward(self, H, X_neis, V):
        ...
        mask = mask.float() - torch.unsqueeze(
            torch.range(0, V - 1).to(device).float(), 1
        )
        mask = (mask == 0).float()
...
class OriLinearGNN(nn.Module):
    ...
    def forward(self, feat_Matrix, X_Node, X_Neis, edge_type_index, dg_list):
        ...
        node_embeds = torch.index_select(
            input=feat_Matrix, dim=0, index=X_Node_decrease
        )  # (N, ln)
        neis_embeds = torch.index_select(
            input=feat_Matrix, dim=0, index=X_Neis_decrease
        )  # (N, ln)

        edge_type_embeds = torch.zeros(
            (edge_type_index.shape[0], 10), dtype=torch.float32
        )
...
class Hybrid_Network(nn.Module):
    def __init__(self, feat_dim, stat_dim, T):
        ...
        self.linear1 = nn.Linear(in_features=64, out_features=32, bias=True)
        self.linear = nn.Linear(in_features=64, out_features=2, bias=True)
...
def test(test):
    ...
    model.load_state_dict(
        torch.load(
            "/Users/trucdiep/DeepCatra/DeepCatra/model/model_best_params.pkl",
            map_location=device,
        )
    )
    model.to(device)
    ...
    with torch.no_grad():
        ...
        # accuracy:  0.9389848812095032


# precision:  0.9652855543113102
# recall:  0.913135593220339
# f1-score:  0.9384866630375612
# auc:  0.04044323527215708


def main():
    test_dataset_path = "/Users/trucdiep/DeepCatra/Dataset/valid_dataset/"
    # sys.argv[1]
